...
> **Note:** Remote Control is in research preview and available on all plans. On Team and Enterprise, it is off by default until an admin enables the Remote Control toggle in Claude Code admin settings.
...
You can start a Remote Control session from the CLI or the VS Code extension.
...
| `--spawn <mode>` | How the server creates sessions. `same-dir` (default): all sessions share the current working directory. `worktree`: each on-demand session gets its own git worktree. Requires a git repository. `session`: single-session mode. Press `w` at runtime to toggle between `same-dir` and `worktree`. |
...
Claude decides when to push. It typically sends one when a long-running task finishes or when it needs a decision from you to continue. You can also request a push in your prompt, for example `notify me when the tests finish`.

> **Note:** Mobile push notifications require Claude Code v2.1.110 or later.

To set up mobile push notifications:

1. **Install the Claude mobile app** — download the Claude app for iOS or Android.
...
- If `/config` shows **No mobile registered**, open the Claude app on your phone so it can refresh its push token.
...
- **Some commands are local-only**: commands that open an interactive picker in the terminal, such as `/mcp`, `/plugin`, or `/resume`, work only from the local CLI.
...
- **The error mentions `disableRemoteControl`**: your IT administrator has disabled Remote Control on this device through managed settings.
...
- Not signed in: run `claude` and use `/login` to authenticate with your claude.ai account.
...
- Session creation failed: if you also see `Session creation failed — see debug log`, check that your subscription is active.
...
| Channels | Push events from a chat app like Telegram or Discord | Your machine (CLI) | Install a channel plugin or build your own | Reacting to external events like CI failures or chat messages |
